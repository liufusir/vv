...
1. prop __被用于传入初始值；而子组件想在之后将其作为一个局部数据属性__。在这种情况下，最好是新定义一个局部数据属性，从 props 上获取初始值即可：
...
```
2. __需要对传入的 prop 值做进一步的转换__。在这种情况中，最好是基于该 prop 值定义一个计算属性：
...
`defineProps()` 宏中的参数不可以访问 `<script setup>` __中定义的其他变量__，因为在编译时整个表达式都会被移到外部的函数中。
:::
